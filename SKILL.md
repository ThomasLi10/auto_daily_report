...
- **远程机器的会话**：别的机器上的 Claude 会话（如 Windows 机上的 Claude Desktop，ssh 进去是 msys2 bash）用 `DAILY_REPORT_SSH_HOSTS="host1 host2"`（ssh 别名，cron 经 `daily_report.local` 注入）或 `--ssh-hosts` 纳入。每台一次 `ssh -o BatchMode=yes`，只拉窗口起点之后改过的 `~/.claude/projects/*/*.jsonl`，**不读远程 git**。会话带 `[<host>]` 标签，同样和本机同主题工作**合并**；连不上时跳过并打 `# NOTE`。
